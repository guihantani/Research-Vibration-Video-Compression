...
def video_compression(video_path, number_components):

    #beam_number_components = 8
    structure_number_components = 12
    three_beams_number_components = 20

    #number_components = number_of_components
    ...
    factors = np.ones(number_components) * 15
    ...
    for result in range(number_of_modes + 1):
        magnification.create_modes_from_frames("mode%d" % result, frames=mother_matrix[result])

    # saving matrices
    mode_shapes = magnification.mode_shapes.astype("int16")
    h5py_tools.save_matrix(mode_shapes.astype("float16"), "binary_files/mode_shapes")
    h5py_tools.save_matrix(magnification.modal_coordinates.astype("float16"), "binary_files/modal_coordinates")
    background = np.copy(magnification.time_serie_mean.astype("uint8"))
    h5py_tools.save_matrix(background, "binary_files/background")

    # reading matrices
    magnification.mode_shapes = h5py_tools.read_matrix("binary_files/mode_shapes")
    magnification.modal_coordinates = h5py_tools.read_matrix("binary_files/modal_coordinates")
    magnification.time_serie_mean = h5py_tools.read_matrix("binary_files/background")
